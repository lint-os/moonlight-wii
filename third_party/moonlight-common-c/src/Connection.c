...
int LiStartConnection(PSERVER_INFORMATION serverInfo, PSTREAM_CONFIGURATION streamConfig, PCONNECTION_LISTENER_CALLBACKS clCallbacks,
    ...
    printf("[LiStart] codecModeSupport=%d appVersion='%s' rtspUrl='%s' drCaps=0x%x submitDecodeUnit=%p arCaps=0x%x\n",
           serverInfo ? serverInfo->serverCodecModeSupport : -1,
           (serverInfo && serverInfo->serverInfoAppVersion) ? serverInfo->serverInfoAppVersion : "(null)",
           (serverInfo && serverInfo->rtspSessionUrl) ? serverInfo->rtspSessionUrl : "(null)",
           drCallbacks ? (int)drCallbacks->capabilities : -1,
           (drCallbacks && drCallbacks->submitDecodeUnit) ? (void*)1 : (void*)0,
           arCallbacks ? (int)arCallbacks->capabilities : -1);
