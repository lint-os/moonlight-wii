...
static void AudioPingThreadProc(void* context) {
    ...
    while (!PltIsThreadInterrupted(&udpPingThread)) {
        ...
        if (AudioPingPayload.payload[0] != 0) {
            ...
            pret = (int)netcompat_sendto(rtpSocket, (char*)&AudioPingPayload, sizeof(AudioPingPayload), 0, (struct sockaddr*)&saddr, AddrLen);
        }
        else {
        ...
        if (pingCount <= 4)
            Limelog("[audio-diag] ping #%d sendto ret=%d errno=%d -> port %u (addrlen %d)\n",
                    pingCount, pret, (int)LastSocketError(), AudioPortNumber, (int)AddrLen);
...
void stopAudioStream(void) {
    ...
    Limelog("[teardown] audio: joining receive\n");
    PltJoinThread(&receiveThread);
    if ((AudioCallbacks.capabilities & CAPABILITY_DIRECT_SUBMIT) == 0) {
        Limelog("[teardown] audio: joining decoder\n");
