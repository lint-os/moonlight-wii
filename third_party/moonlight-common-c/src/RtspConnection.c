...
#define RTSP_CONNECT_TIMEOUT_SEC 30
...
static char* sealRtspMessage(PRTSP_MESSAGE request, int* messageLen) {
    ...
    if (serializedMessage == NULL) {
        Limelog("[rtsp] seal: serializeRtspMessage failed\n");
        return NULL;
    }
    ...
    if (encryptedMessage == NULL) {
        Limelog("[rtsp] seal: malloc failed (len %d)\n", plaintextLen);
    ...
    // The payload region (right after the header) is exactly plaintextLen bytes;
    // GCM ciphertext is the same length as the plaintext. Tell the crypto layer
    // the output capacity up front, since it reads *messageLen as the capacity.
    *messageLen = plaintextLen;
    ...
    if (!success) {
        Limelog("[rtsp] seal: PltEncryptMessage failed (len %d)\n", plaintextLen);
...
static bool unsealRtspMessage(char* rawMessage, int rawMessageLen, PRTSP_MESSAGE response) {
    ...
    // Diagnostic: dump the raw RTSP response (first 16 bytes) to see exactly what
    // the server sent and where parsing fails.
    {
        int hd = rawMessageLen < 16 ? rawMessageLen : 16;
        printf("[rtsp] unseal raw=%d bytes:", rawMessageLen);
        for (int i = 0; i < hd; i++) {
            printf(" %02x", (unsigned char)rawMessage[i]);
        }
        printf("\n");
    }
...
static bool transactRtspMessageTcp(PRTSP_MESSAGE request, PRTSP_MESSAGE response, int* error) {
    ...
    *error = -1;
    ...
    Limelog("[rtsp] sent %d/%d bytes, waiting for response (timeout %ds)\n",
            err, messageLen, RTSP_RECEIVE_TIMEOUT_SEC);

    // Read the response until the server closes the connection.
    //
    // On the Wii, libogc's net_select() (used by poll()) does not reliably
    // report incoming data on this socket, and its net_ioctl() only supports
    // FIONBIO (no FIONREAD). So we can't wait for data with poll() or
    // FIONREAD. Instead we make the socket non-blocking and simply retry
    // recv(): it returns the data when the server responds, or EAGAIN when
    // nothing has arrived yet, so we sleep briefly and retry until the
    // timeout elapses.
    {
    ...
    {
        int waitRemainingMs = RTSP_RECEIVE_TIMEOUT_SEC * 1000;
        for (;;) {
            ...
            if (err > 0) {
                offset += err;
                Limelog("[rtsp] recv %d bytes (total %d)\n", err, offset);
            }
            else if (err == 0) {
                // Done reading (server closed the connection)
                break;
            }
            else {
                int e = LastSocketError();
                if (e == EAGAIN || e == EWOULDBLOCK || e == EINTR) {
                    // No data yet; wait a bit and retry until the timeout.
...
int performRtspHandshake(PSERVER_INFORMATION serverInfo) {
    ...
    decryptionCtx = PltCreateCryptoContext();
    {
        char kh[33] = { 0 };
        for (int i = 0; i < 16; i++)
            sprintf(kh + i * 2, "%02x", (unsigned char)StreamConfig.remoteInputAesKey[i]);
        Limelog("[rtsp] encryptedRtspEnabled=%d key=%s\n", encryptedRtspEnabled, kh);
    }
