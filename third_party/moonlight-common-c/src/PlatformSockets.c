...
#ifdef __WII__
// lwIP (the Wii's IOS socket stack) rejects bind() to port 0 (wildcard/ephemeral)
// with EINVAL, so assign each UDP socket an explicit local port.
in_port_t wii_udp_port = 49152;
#endif
...
SOCKET bindUdpSocket(int addressFamily, struct sockaddr_storage* localAddr, SOCKADDR_LEN addrLen, int bufferSize, int socketQosType) {
    ...
    struct sockaddr_in *wii_addr = (struct sockaddr_in *)&bindAddr;
    wii_addr->sin_port = htons(wii_udp_port++);
#endif
...
SOCKET connectTcpSocket(struct sockaddr_storage* dstaddr, SOCKADDR_LEN addrlen, unsigned short port, int timeoutSec) {
    ...
    // Create a blocking TCP socket. On the Wii (libogc/lwIP) a non-blocking
    // connect returns EINPROGRESS and its completion event is not reliably
    // delivered to net_select(), so the connect-wait poll times out. Connect
    // blocking instead; non-blocking I/O is re-disabled right after anyway.
    s = createSocket(dstaddr->ss_family, SOCK_STREAM, IPPROTO_TCP, false);
    if (s == INVALID_SOCKET) {
        return INVALID_SOCKET;
    }
    ...
    if (err < 0) {
        ...
        // A blocking connect (returns 0) skips this entirely -- libogc's
        // net_select() does not report an already-connected socket as ready,
        // so polling it would time out.
        pfd.fd = s;
        pfd.events = POLLOUT;
