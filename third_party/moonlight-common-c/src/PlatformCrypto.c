...
bool PltEncryptMessage(PPLT_CRYPTO_CONTEXT ctx, int algorithm, int flags,
    ...
    if (!ctx->initialized) {
        int r = mbedtls_cipher_setup(&ctx->ctx, mbedtls_cipher_info_from_values(MBEDTLS_CIPHER_ID_AES, keyLength * 8, cipherMode));
        if (r != 0) {
            Limelog("[crypto] cipher_setup failed: -0x%04x\n", (unsigned int)-r);
            return false;
        }

        r = mbedtls_cipher_setkey(&ctx->ctx, key, keyLength * 8, MBEDTLS_ENCRYPT);
        if (r != 0) {
            Limelog("[crypto] cipher_setkey failed: -0x%04x\n", (unsigned int)-r);
            return false;
        }
...
#else
        {
            int r = mbedtls_cipher_auth_encrypt(&ctx->ctx, iv, ivLength, NULL, 0, inputData, inputDataLength, outputData, &outLength, tag, tagLength);
            if (r != 0) {
                Limelog("[crypto] cipher_auth_encrypt failed: -0x%04x (inlen %d, ivlen %d, outcap %u)\n",
                        (unsigned int)-r, inputDataLength, ivLength, (unsigned int)outLength);
                return false;
            }
        }
#endif
