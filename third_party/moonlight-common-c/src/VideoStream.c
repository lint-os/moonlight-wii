...
// Wii: libogc sendto() passes the 16-byte sockaddr_in straight to the IOS,
// which rejects it (EINVAL). netcompat_sendto() normalizes to the 8-byte
// wii_sockaddr_in layout the real-HW IOS expects.
extern int netcompat_sendto(int sockfd, const void *buf, int len, int flags,
...
static void VideoPingThreadProc(void* context) {
    ...
    while (!PltIsThreadInterrupted(&udpPingThread)) {
        ...
        if (VideoPingPayload.payload[0] != 0) {
            ...
            pret = (int)netcompat_sendto(rtpSocket, (char*)&VideoPingPayload, sizeof(VideoPingPayload), 0, (struct sockaddr*)&saddr, AddrLen);
        }
        else {
            pret = (int)netcompat_sendto(rtpSocket, legacyPingData, sizeof(legacyPingData), 0, (struct sockaddr*)&saddr, AddrLen);
        }

        if (pingCount <= 4) {
            Limelog("[video-diag] ping #%d sendto ret=%d errno=%d -> port %u (addrlen %d)\n",
                    pingCount, pret, (int)LastSocketError(), VideoPortNumber, (int)AddrLen);
        }
...
#ifdef __WII__
    {
        struct sockaddr_storage vss;
        socklen_t vlen = sizeof(vss);
        if (getsockname(rtpSocket, (struct sockaddr*)&vss, &vlen) == 0 && vss.ss_family == AF_INET)
            Limelog("[video-diag] rtpSocket bound to port %u\n", ntohs(((struct sockaddr_in*)&vss)->sin_port));
        else
            Limelog("[video-diag] rtpSocket getsockname failed\n");
    }
#endif
...
#ifdef __WII__
                if (waitingForVideoMs % 2000 < UDP_RECV_POLL_TIMEOUT_MS)
                    Limelog("[video-diag] still waiting for video: %d ms\n", waitingForVideoMs);
#endif
...
static void VideoDecoderThreadProc(void* context) {
    ...
    while (!PltIsThreadInterrupted(&decoderThread)) {
        ...
        if (!LiWaitForNextVideoFrame(&frameHandle, &decodeUnit)) {
            Limelog("[teardown] video decoder: exiting (queue shutdown)\n");
            return;
        }
        ...
        if (++frameCount % 150 == 0)
            Limelog("[teardown] video decoder: alive, %d frames decoded\n", frameCount);
    }
    Limelog("[teardown] video decoder: loop exited\n");
}
...
void stopVideoStream(void) {
    ...
    Limelog("[teardown] video: joining ping\n");
    PltJoinThread(&udpPingThread);
    Limelog("[teardown] video: joining receive\n");
    PltJoinThread(&receiveThread);
    if ((VideoCallbacks.capabilities & (CAPABILITY_DIRECT_SUBMIT | CAPABILITY_PULL_RENDERER)) == 0) {
        Limelog("[teardown] video: joining decoder\n");
