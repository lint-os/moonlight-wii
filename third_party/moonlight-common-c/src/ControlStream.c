...
#ifdef __WII__
        {
            // Periodically dump the live peer state so a real-HW run shows whether
            // the RTT ever updates from its 500ms default and when loss spikes.
            static uint64_t lastStatsLog = 0;
            uint64_t now = PltGetMillis();
            if (now - lastStatsLog >= 2000) {
                lastStatsLog = now;
                Limelog("[enet-diag] live: state=%d sent=%u lost=%u rtt=%u rttVar=%u lastRecvAge=%u ms\n",
                        (int)peer->state,
                        (unsigned)peer->packetsSent,
                        (unsigned)peer->packetsLost,
                        (unsigned)peer->roundTripTime,
                        (unsigned)peer->roundTripTimeVariance,
                        (unsigned)ENET_TIME_DIFFERENCE(client->serviceTime, peer->lastReceiveTime));
            }
        }
#endif
...
#ifdef __WII__
            {
                // event.data carries the server's disconnect reason for a *received*
                // disconnect; a local reliable-ACK timeout has data==0. Read the peer
                // stats BEFORE the state is fully torn down.
                Limelog("[enet-diag] disconnect: event.data=0x%x state=%d sent=%u lost=%u rtt=%u rttVar=%u lastRecvAge=%u ms minT=%u maxT=%u\n",
                        (unsigned)event.data,
                        (int)peer->state,
                        (unsigned)peer->packetsSent,
                        (unsigned)peer->packetsLost,
                        ...
                        (unsigned)peer->roundTripTimeVariance,
                        (unsigned)ENET_TIME_DIFFERENCE(client->serviceTime, peer->lastReceiveTime),
                        (unsigned)peer->timeoutMinimum,
                        (unsigned)peer->timeoutMaximum);
            }
#endif
            ListenerCallbacks.connectionTerminated(-1);
            return;
...
int stopControlStream(void) {
    ...
    Limelog("[teardown] control: joining lossStats\n");
    PltJoinThread(&lossStatsThread);
    Limelog("[teardown] control: joining reqIdrFrame\n");
    PltJoinThread(&requestIdrFrameThread);
    Limelog("[teardown] control: joining controlReceive\n");
    PltJoinThread(&controlReceiveThread);
    Limelog("[teardown] control: joining asyncCallback\n");
    ...
    if (isReferenceFrameInvalidationEnabled()) {
        ...
        Limelog("[teardown] control: joining invalidateRefFrames\n");
...
int startControlStream(void) {
    ...
    if (AppVersionQuad[0] >= 5) {
        ...
        // lwIP (Wii IOS) rejects bind() to port 0 (wildcard), so assign an explicit port
        enet_address_set_port(&localAddress, htons(wii_udp_port++));
#else
...
#ifdef __WII__
        {
            char la[64] = "?", ra[64] = "?", ba[64] = "?";
            struct sockaddr_storage bss; socklen_t blen = sizeof(bss);
            if (LocalAddr.ss_family == AF_INET) inet_ntop(AF_INET, &((struct sockaddr_in*)&LocalAddr)->sin_addr, la, sizeof(la));
            else if (LocalAddr.ss_family == AF_INET6) inet_ntop(AF_INET6, &((struct sockaddr_in6*)&LocalAddr)->sin6_addr, la, sizeof(la));
            else { la[0] = '0'; la[1] = 0; }
            if (RemoteAddr.ss_family == AF_INET) inet_ntop(AF_INET, &((struct sockaddr_in*)&RemoteAddr)->sin_addr, ra, sizeof(ra));
            else if (RemoteAddr.ss_family == AF_INET6) inet_ntop(AF_INET6, &((struct sockaddr_in6*)&RemoteAddr)->sin6_addr, ra, sizeof(ra));
            else { ra[0] = '0'; ra[1] = 0; }
            if (getsockname(client->socket, (struct sockaddr*)&bss, &blen) == 0) {
                if (bss.ss_family == AF_INET) inet_ntop(AF_INET, &((struct sockaddr_in*)&bss)->sin_addr, ba, sizeof(ba));
                else if (bss.ss_family == AF_INET6) inet_ntop(AF_INET6, &((struct sockaddr_in6*)&bss)->sin6_addr, ba, sizeof(ba));
            } else { ba[0] = 'F'; ba[1] = 0; }
            Limelog("[enet-diag] LocalAddr fam=%d ip=%s | RemoteAddr fam=%d ip=%s | bound fam=%d ip=%s\n",
                    (int)LocalAddr.ss_family, la, (int)RemoteAddr.ss_family, ra, (int)bss.ss_family, ba);
        }
#endif
