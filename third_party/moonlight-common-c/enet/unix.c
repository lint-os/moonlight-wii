...
#if defined(__WII__)
#include <network.h>
// netcompat.c (src/wii) translates a public socket() handle into the internal
// net_* index. The Wii IOS stack is driven through the net_* API directly
// (mirroring libogc's udptest example): net_socket/net_connect/net_send/
// net_recv with flags=0. Passing any non-zero flag (e.g. MSG_NOSIGNAL) makes
// the IOS ioctl return EOPNOTSUPP, so we always pass 0.
extern int netcompat_netfd(int fd);

// The Wii's IOS UDP stack only supports connected UDP (connect + send/recv);
// sendto/recvfrom with an explicit address are rejected. Track the connected
// peer and the internal net_* index of the socket it belongs to.
static int wiiUdpSock = -1;
static int wiiUdpNetFd = -1;
static ENetAddress wiiUdpPeer;
// Diagnostics: cumulative traffic counters so a real-HW run can show whether
// both directions keep flowing up to (and past) the disconnect.
static enet_uint32 wiiSentPackets = 0;
static enet_uint32 wiiSentBytes = 0;
static enet_uint32 wiiRecvPackets = 0;
static enet_uint32 wiiRecvBytes = 0;
#endif
...
{
#if defined(__WII__)
    // A closed socket invalidates the cached internal net_* index. Without this
    // reset, a re-stream whose new ENet socket reuses the same public handle
    // (and the same server port) would skip the net_connect() re-detect and
    // net_send() the stale, already-closed fd (seen: net_send ret=-121).
    if (wiiUdpSock == (int)socket)
    {
        wiiUdpSock = -1;
        wiiUdpNetFd = -1;
    }
                  ...
                  size_t bufferCount)
{
...
#if defined(__WII__)
    {
        int nfd = netcompat_netfd((int) socket);
        if (nfd < 0)
        {
            if (bufferCount > 1)
              free(sendBuffer);
        ...
        // Re-connect whenever the *internal* net fd or the peer changed. net_send
        // drives the internal fd, and the public handle value can be reused after
        // close(), so keying the cache on the public handle alone would net_send()
        // a stale, already-closed internal fd on a re-stream (seen: net_send
        // ret=-121 on the 2nd connect). The internal fd of a fresh socket differs
        // from the previous session's, so this forces a net_connect on re-stream.
        if (nfd != wiiUdpNetFd || !enet_address_equal ((ENetAddress *) peerAddress, & wiiUdpPeer))
        {
            // net_connect wants the 8-byte wii_sockaddr_in layout (len, family,
            // port, addr); the first 8 bytes of our address are exactly that.
            struct sockaddr_in sin;
            memcpy(&sin, &peerAddress->address, sizeof(sin));
            int cr = net_connect(nfd, (struct sockaddr *) & sin, 8);
            if (cr < 0)
            {
                printf("[enet] net_connect failed ret=%d\n", cr);
                if (bufferCount > 1)
                  free(sendBuffer);
                return -1;
            }
            wiiUdpSock = (int)socket;
            wiiUdpNetFd = nfd;
            wiiUdpPeer = * peerAddress;
        }

        sentLength = net_send(wiiUdpNetFd, sendBuffer, (s32) sendLength, 0);
        if (sentLength < 0)
        {
            printf("[enet] net_send failed ret=%d len=%d (sent %u pkts/%u B, recv %u pkts/%u B)\n",
                   sentLength, (int)sendLength,
                   wiiSentPackets, wiiSentBytes, wiiRecvPackets, wiiRecvBytes);
            if (bufferCount > 1)
              free(sendBuffer);
            if (sentLength == -EAGAIN || sentLength == -EWOULDBLOCK)
              return 0;
            return -1;
        }
        ...
        ++wiiSentPackets;
        wiiSentBytes += (enet_uint32) sentLength;
        if ((wiiSentPackets + wiiRecvPackets) % 64 == 0)
          printf("[enet] traffic: sent %u pkts/%u B, recv %u pkts/%u B\n",
                 wiiSentPackets, wiiSentBytes, wiiRecvPackets, wiiRecvBytes);
        return sentLength;
    }
#else
    ...
    {
      char ipbuf[64] = "?";
      if (peerAddress->address.ss_family == AF_INET)
        inet_ntop(AF_INET, &((struct sockaddr_in*)&peerAddress->address)->sin_addr, ipbuf, sizeof(ipbuf));
      else if (peerAddress->address.ss_family == AF_INET6)
        inet_ntop(AF_INET6, &((struct sockaddr_in6*)&peerAddress->address)->sin6_addr, ipbuf, sizeof(ipbuf));
      printf("[enet] sendto failed errno=%d len=%d sock=%d fam=%d addrlen=%d ip=%s\n",
             errno, (int)sendLength, (int)socket, (int)peerAddress->address.ss_family,
             (int)peerAddress->addressLength, ipbuf);
    }
#else
                     ...
                     size_t bufferCount)
{
...
#if defined(__WII__)
    {
        ...
        recvLength = net_recv(nfd, buffers[0].data, (s32) buffers[0].dataLength, 0);
        if (recvLength < 0)
        {
           if (recvLength == -EAGAIN || recvLength == -EWOULDBLOCK)
             return 0;

           printf("[enet] net_recv failed ret=%d (sent %u pkts/%u B, recv %u pkts/%u B)\n",
                  recvLength, wiiSentPackets, wiiSentBytes, wiiRecvPackets, wiiRecvBytes);
           return -1;
        }
        if (wiiUdpSock == (int)socket)
          * peerAddress = wiiUdpPeer;

        ++wiiRecvPackets;
        wiiRecvBytes += (enet_uint32) recvLength;
        if ((wiiSentPackets + wiiRecvPackets) % 64 == 0)
          printf("[enet] traffic: sent %u pkts/%u B, recv %u pkts/%u B\n",
                 wiiSentPackets, wiiSentBytes, wiiRecvPackets, wiiRecvBytes);
        return recvLength;
    ...
    if (recvLength == -1)
    {
       ...
       printf("[enet] recvfrom failed errno=%d\n", errno);
       return -1;
    }
