...
enet_protocol_handle_disconnect (ENetHost * host, ENetPeer * peer, const ENetProtocol * command)
{
    ...
    printf("[enet-diag] RECEIVED disconnect from server: state=%d data=0x%x\n",
           (int)peer -> state, (unsigned)ENET_NET_TO_HOST_32 (command -> disconnect.data));
...
enet_protocol_handle_acknowledge (ENetHost * host, ENetEvent * event, ENetPeer * peer, const ENetProtocol * command)
{
    ...
    case ENET_PEER_STATE_DISCONNECTING:
       ...
       printf("[enet-diag] LOCAL disconnect ACKed by server\n");
...
enet_protocol_check_timeouts (ENetHost * host, ENetPeer * peer, ENetEvent * event)
{
    ...
    while (currentCommand != enet_list_end (& peer -> sentReliableCommands))
    {
       ...
       if (peer -> earliestTimeout != 0 &&
        ...
        {
           printf("[enet-diag] LOCAL reliable-ACK timeout: earliestAge=%u ms minT=%u maxT=%u\n",
                  (unsigned)ENET_TIME_DIFFERENCE (host -> serviceTime, peer -> earliestTimeout),
                  (unsigned)peer -> timeoutMinimum, (unsigned)peer -> timeoutMaximum);
           enet_protocol_notify_disconnect (host, peer, event);
